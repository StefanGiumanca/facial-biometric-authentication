        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            <DetailRow
              ...
              valueColor={getStatusColor(session.status)}
            />
            <DetailRow
            ...
            <DetailRow
              ...
              valueColor={getStatusColor(session.selfie_gate_decision || '')}
            />
            <DetailRow
              label="Final Face Check"
              value={formatDecisionWithDistance(session.final_face_match_decision, session.final_face_match_distance)}
              valueColor={getStatusColor(session.final_face_match_decision || '')}
            />
            <DetailRow
              label="Legacy Face Match"
              value={formatDecisionWithDistance(session.face_match_decision, session.face_match_distance)}
              valueColor={getStatusColor(session.face_match_decision || '')}
            />
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            <DetailRow
              ...
              valueColor={(session.security_fail_count ?? 0) > 0 ? '#F59E0B' : '#10B981'}
            />
            <DetailRow label="Reject Reason" value={session.reject_reason || 'N/A'} />
            <DetailRow label="Locked At" value={session.locked_at ? formatDate(session.locked_at) : 'N/A'} />
          </View>
        </View>
...
function formatDecisionWithDistance(decision?: string | null, distance?: number | null) {
  if (!decision) {
    return 'N/A';
  }

  return typeof distance === 'number' ? `${decision} (${distance.toFixed(3)})` : decision;
}
