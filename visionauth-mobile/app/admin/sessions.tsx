...
export default function AdminSessionsScreen() {
  ...
  const renderSessionCard = ({ item }: { item: AdminSession }) => {
    ...
    const displayStatus = item.final_decision || item.status || 'IN_PROGRESS';
    ...
    return (
      <Pressable
        ...
        {item.selfie_gate_decision && (
          <View style={styles.cardDetails}>
            <Text style={styles.detailLabel}>Selfie Gate:</Text>
            <Text style={styles.detailValue}>
              {item.selfie_gate_decision}
              {typeof item.selfie_gate_distance === 'number' ? ` (${item.selfie_gate_distance.toFixed(3)})` : ''}
            </Text>
          </View>
        )}

        {item.final_face_match_decision && (
          <View style={styles.cardDetails}>
            <Text style={styles.detailLabel}>Final Check:</Text>
            <Text style={styles.detailValue}>
              {item.final_face_match_decision}
              {typeof item.final_face_match_distance === 'number' ? ` (${item.final_face_match_distance.toFixed(3)})` : ''}
            </Text>
          </View>
        )}
        ...
        <View style={styles.cardDetails}>
          ...
          <Text style={[styles.detailValue, { color: getStatusColor(item.status) }]}>{item.status}</Text>
        </View>

        <View style={styles.cardDetails}>
          <Text style={styles.detailLabel}>Security Fails:</Text>
          <Text style={[styles.detailValue, { color: item.security_fail_count > 0 ? '#F59E0B' : '#10B981' }]}>
            {item.security_fail_count}/3
          </Text>
        </View>
