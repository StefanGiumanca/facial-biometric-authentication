...
from backend.models.database_models import AuditLog, KycSession


# Constants
MAX_SECURITY_FAILURES = 3

# Selfie upload uses a strict gate: only clear matches are accepted.
SELFIE_GATE_ACCEPT_DISTANCE = 0.50
SELFIE_GATE_REVIEW_DISTANCE = 0.60

# Final face match can still surface manual review before a hard reject.
FINAL_FACE_MATCH_ACCEPT_DISTANCE = 0.50
FINAL_FACE_MATCH_REVIEW_DISTANCE = 0.60
FACE_MATCH_THRESHOLD = FINAL_FACE_MATCH_REVIEW_DISTANCE
...
def handle_final_face_match_failure(session_id: str, result: dict) -> dict:
    """
    Register a failed final face-match attempt using the shared security policy.

    This helper keeps the route code small and preserves the audit metadata that
    the admin view expects when the comparison is fully rejected.
    """
    return register_security_failure(
        session_id,
        "FACE_MATCH_FAILED",
        {
