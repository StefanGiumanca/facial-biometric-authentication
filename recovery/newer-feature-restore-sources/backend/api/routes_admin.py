...
def admin_get_sessions(
    ...
    - liveness_passed
    - selfie_gate_distance, selfie_gate_decision (from security gate step)
    - final_face_match_distance, final_face_match_decision (from final check)
    - face_match_distance, face_match_decision (legacy)
    - final_decision
    - status (STARTED, IN_PROGRESS, APPROVED, MANUAL_REVIEW, REJECTED, etc.)
    - security_fail_count: number of security failures for this session
    - reject_reason: reason for rejection (if locked)
    - locked_at: timestamp when session was locked (if applicable)
    """
...
def admin_get_session_detail(
    ...
    - Verification results:
      ...
      * selfie_gate_distance, selfie_gate_decision: from security gate (more permissive)
      * final_face_match_distance, final_face_match_decision: from final check (strict)
      * face_match_distance, face_match_decision: legacy field
      * final_decision: APPROVED, MANUAL_REVIEW, REJECTED, or null
    - Raw OCR text, timestamps
    - Security fields:
      * security_fail_count: total security failures
      * reject_reason: reason for rejection (if locked)
      * locked_at: timestamp when session was locked
      * status: STARTED, IN_PROGRESS, APPROVED, MANUAL_REVIEW, REJECTED, or FRAUD
    - embeddings: metadata of stored face embeddings (without full vectors)
    """
    verify_admin_key(x_admin_key)
